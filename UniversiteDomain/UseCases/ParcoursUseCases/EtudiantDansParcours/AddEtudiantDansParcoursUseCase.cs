...
public class AddEtudiantDansParcoursUseCase(IRepositoryFactory repositoryFactory)
{
    ...
    {
        long[] idEtudiants = etudiants.Select(x => x.EtudiantId).ToArray();
        return await ExecuteAsync(parcours.ParcoursId, idEtudiants);
    }

    private async Task<Parcours> ExecuteAsync(long idParcours, long[] idEtudiants)
    {
    ...
    private async Task CheckBusinessRules(long idParcours, long idEtudiant)
    {
        ...
        var etudiant =
            await repositoryFactory.EtudiantRepository().FindByConditionAsync(e => e.EtudiantId.Equals(idEtudiant));
